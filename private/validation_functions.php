<?php
  ...
  function reg_valid($user,$email,$password){
    if (is_blank($user)) {
        redirect_to_register("user name can't be blank");
    }
    if(!has_length($user, ['min' => 1, 'max' => 25])){
        redirect_to_register("user name has to be between 3 and 25 chars");
    }
    if(!is_unique_name($user)){
      redirect_to_register("user name must be unique");
    };
    
    if (is_blank($email)) {
        redirect_to_register("email can't be blank");
    }
    if(!has_valid_email_format($email)){
        redirect_to_register("email not valid");
    }
